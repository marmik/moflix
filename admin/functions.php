...
function get_songs($connect){

$sentence = $connect->prepare("SELECT SQL_CALC_FOUND_ROWS songs.*, GROUP_CONCAT(genres.genre_title) AS genre_title FROM songs JOIN songs_genres ON songs_genres.song_id = songs.song_id JOIN genres ON genres.genre_id = songs_genres.genre_id GROUP BY songs.song_id ORDER BY songs.song_date DESC LIMIT 8");

    $sentence->execute();
    return $sentence->fetchAll(PDO::FETCH_ASSOC);
}

function get_all_songs($connect)
{

$sentence = $connect->prepare("SELECT SQL_CALC_FOUND_ROWS songs.*, GROUP_CONCAT(genres.genre_title) AS genre_title FROM songs JOIN songs_genres ON songs_genres.song_id = songs.song_id JOIN genres ON genres.genre_id = songs_genres.genre_id GROUP BY songs.song_id ORDER BY songs.song_date DESC");

    $sentence->execute();
    return $sentence->fetchAll(PDO::FETCH_ASSOC);
}

function id_song($id_song){
    return (int)cleardata($id_song);
}

function get_song_per_id($connect, $id_song){
    $sentence = $connect->query("SELECT SQL_CALC_FOUND_ROWS songs.*, GROUP_CONCAT(genres.genre_title) AS genre_title FROM songs JOIN songs_genres ON songs_genres.song_id = songs.song_id JOIN genres ON genres.genre_id = songs_genres.genre_id WHERE songs.song_id = $id_song GROUP BY songs.song_id LIMIT 1");
    $sentence = $sentence->fetchAll();
    return ($sentence) ? $sentence : false;
}

function get_song_slug($connect, $slug){

$sentence = $connect->prepare("SELECT * FROM songs WHERE song_slug = $slug");
$sentence->execute();
$row = $sentence->fetch(PDO::FETCH_ASSOC);
return $row[0];
}

function number_songs($connect){

$total_numbers = $connect->prepare("SELECT * FROM songs");
$total_numbers->execute(array());
$total_numbers->fetchAll();
$total = $total_numbers->rowCount();
return $total;
...
function selected_song_genres($connect){

if (isset($_GET['id']) && !empty($_GET['id'])) {

$id = intval($_GET['id']);

$sentence = $connect->prepare("SELECT genres.genre_title, genres.genre_id FROM genres JOIN songs_genres ON songs_genres.genre_id = genres.genre_id JOIN songs ON songs_genres.song_id = ? GROUP BY songs_genres.genre_id");
$sentence->execute([$id]);
return $sentence->fetchAll();
...
/*function get_song_per_id($connect, $id) {
    $statement = $connect->prepare("SELECT * FROM songs WHERE song_id = :id");
    $statement->execute(array(':id' => $id));
    $result = $statement->fetchAll();
    return $result;
}

function selected_songgenres($connect, $id) {
    $statement = $connect->prepare("
        SELECT genres.genre_id, genres.genre_name 
        FROM songs_genres 
        INNER JOIN genres ON songs_genres.genre_id = genres.genre_id 
        WHERE songs_genres.song_id = :id
    ");
    $statement->execute(array(':id' => $id));
    $result = $statement->fetchAll();
    return $result;
...
function not_selected_songgenres($connect, $id) {
    $statement = $connect->prepare("
        SELECT genres.genre_id, genres.genre_name 
        FROM genres 
        WHERE NOT EXISTS (
            SELECT * 
            FROM songs_genres 
            WHERE songs_genres.genre_id = genres.genre_id 
            AND songs_genres.song_id = :id
        )
    ");
    $statement->execute(array(':id' => $id));
    $result = $statement->fetchAll();
...
*/
function not_selected_song_genres($connect){

    if (isset($_GET['id']) && !empty($_GET['id'])) {
   
   $id = intval($_GET['id']);
   
   $sentence = $connect->prepare("SELECT genres.genre_title, genres.genre_id FROM genres WHERE genres.genre_id NOT IN (SELECT songs_genres.genre_id FROM songs_genres WHERE songs_genres.song_id = ? GROUP BY songs_genres.genre_id)");
   $sentence->execute([$id]);
   return $sentence->fetchAll();
