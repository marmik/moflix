...
function isListen(){
    ...
    return isset($_GET['listen']) && !empty($_GET['listen']) && $_GET['listen'] == '1';
}
...
function gotToListen(){
    
    $url = pathinfo($_SERVER['PHP_SELF'], PATHINFO_FILENAME);

    return htmlspecialchars($url.'?listen=1');
}
...
function getUserReviewBySong($itemId, $connect){

    $emailSession = filter_var(strtolower($_SESSION['user_email']), FILTER_SANITIZE_STRING);

    $sentence = $connect->query("SELECT * FROM songs_reviews WHERE user = '".$emailSession."' AND item = $itemId LIMIT 1");
    $sentence->execute();
    return $sentence->fetch();
}
...
function ReviewsBySong($itemId, $connect){
    $sentence = $connect->prepare("SELECT * FROM songs_reviews WHERE item = $itemId ORDER BY published DESC");
    $sentence->execute();
    return $sentence->fetchAll(PDO::FETCH_ASSOC);
}
...
function getFavSongsByUser($connect){
    $emailSession = filter_var(strtolower($_SESSION['user_email']), FILTER_SANITIZE_STRING);
    $sentence = $connect->prepare("SELECT songs_favorites.*, song_title AS title, song_slug AS slug, song_image AS image FROM songs_favorites LEFT JOIN songs ON songs.song_id = songs_favorites.item WHERE user = '".$emailSession."'");
    $sentence->execute();
    return $sentence->fetchAll(PDO::FETCH_ASSOC);
}
...
function getFavBySong($connect, $itemId){
    $emailSession = filter_var(strtolower($_SESSION['user_email']), FILTER_SANITIZE_STRING);
    $sentence = $connect->query("SELECT * FROM songs_favorites WHERE user = '".$emailSession."' AND item = $itemId LIMIT 1");
    $sentence = $sentence->fetch();
    return ($sentence) ? true : false;
}
...
function getSongById($connect, $idSong)
{
    
    $sentence = $connect->prepare("SELECT SQL_CALC_FOUND_ROWS songs.*, GROUP_CONCAT(genres.genre_title) AS genre_title, AVG(rating) AS song_rate, songs.song_id AS id, songs.song_link AS link, songs.song_iframe AS iframe, songs.song_title AS title, songs.song_description AS description  FROM songs JOIN songs_genres ON songs_genres.song_id = songs.song_id LEFT JOIN songs_reviews ON songs_reviews.item = songs.song_id JOIN genres ON genres.genre_id = songs_genres.genre_id WHERE songs.song_id = $idSong AND songs.song_status = 1 GROUP BY songs.song_id LIMIT 1");
...
function getSongsGenresBySong($connect, $idSong)
{
    
    $sentence = $connect->prepare("SELECT genre_title AS genre FROM genres WHERE genre_id IN (SELECT genre_id FROM songs_genres WHERE song_id = $idSong)");
    $sentence->execute();
    $result = $sentence->fetch()['genre'];
    return $result;
...
function getSongsStarsBySong($connect, $idSong)
{
    
    $sentence = $connect->prepare("SELECT song_stars AS stars FROM songs WHERE song_id = $idSong ORDER BY song_stars ASC");
    $sentence->execute();
    $result = $sentence->fetch()['stars'];
    return $result;
...
function getSongsGenres($connect)
{
    
    $sentence = $connect->prepare("SELECT * FROM genres WHERE genre_id IN (SELECT genre_id FROM songs_genres GROUP BY genre_id) ORDER BY genre_title ASC");

    $sentence->execute();
    return $sentence->fetchAll(PDO::FETCH_ASSOC);
}

function getSongsYears($connect)
{
    
    $sentence = $connect->prepare("SELECT song_year AS year FROM songs GROUP BY song_year ORDER BY song_year ASC");

    $sentence->execute();
    return $sentence->fetchAll(PDO::FETCH_ASSOC);
}

function getFeaturedSongs($connect, $limit)
{
    
    $sentence = $connect->prepare("SELECT SQL_CALC_FOUND_ROWS songs.*, GROUP_CONCAT(genres.genre_title) AS genre_title, AVG(rating) AS song_rate FROM songs JOIN songs_genres ON songs_genres.song_id = songs.song_id LEFT JOIN songs_reviews ON songs_reviews.item = songs.song_id JOIN genres ON genres.genre_id = songs_genres.genre_id WHERE songs.song_featured = 1 AND songs.song_status = 1 GROUP BY songs.song_id ORDER BY songs.song_date DESC LIMIT $limit");

    $sentence->execute();
    return $sentence->fetchAll(PDO::FETCH_ASSOC);
}

function getAllSongs($connect, $itemsPage)
{

$limit = (getNumPage() > 1) ? getNumPage() * $itemsPage - $itemsPage : 0;

$sqlQuery = "SELECT SQL_CALC_FOUND_ROWS songs.*, GROUP_CONCAT(genres.genre_title) AS genre_title FROM songs JOIN songs_genres ON songs_genres.song_id = songs.song_id JOIN genres ON genres.genre_id = songs_genres.genre_id WHERE songs.song_status = 1";
...
$titleGet = getParamsTitle();
$sqlQuery .= " AND songs.song_title LIKE '%".$titleGet."%' OR songs.song_stars LIKE '%".$titleGet."%'";
...
$yearGet = getParamsYear();
$sqlQuery .= " AND songs.song_year = '".$yearGet."'";
...
$genreGet = getParamsGenre();
$sqlQuery .= " AND songs.song_id IN (SELECT song_id FROM songs_genres WHERE genre_id = $genreGet)";

}

$sqlQuery .= " GROUP BY songs.song_id ORDER BY songs.song_date DESC";
...
function getRecentsongs($connect, $limit = NULL)
{
    
    $sentence = $connect->prepare("SELECT SQL_CALC_FOUND_ROWS songs.*, GROUP_CONCAT(genres.genre_title) AS genre_title FROM songs JOIN songs_genres ON songs_genres.song_id = songs.song_id JOIN genres ON genres.genre_id = songs_genres.genre_id WHERE songs.song_status = 1 GROUP BY songs.song_id ORDER BY songs.song_date DESC LIMIT $limit");
...
function getTopsongs($connect, $limit = NULL)
{
    
    $sentence = $connect->prepare("SELECT SQL_CALC_FOUND_ROWS songs.*, AVG(rating) AS song_rate FROM songs JOIN songs_reviews ON songs_reviews.item = songs.song_id WHERE songs.song_status = 1 GROUP BY songs.song_id ORDER BY song_rate DESC LIMIT $limit");
