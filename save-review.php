...
if ($_SERVER['REQUEST_METHOD'] == 'POST'){
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST'){
        ...
        $alreadyReviewed = getUserReviewBySong($item, $connect);
    
    
        if (!$alreadyReviewed) {
    
        $statment = $connect->prepare(
            "INSERT INTO songs_reviews (id, item, user, content, rating, published) VALUES (null, :item, :user, :content, :rating, CURRENT_TIMESTAMP)");
