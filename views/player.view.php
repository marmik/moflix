<?php if(isWatch()): ?>
    ...
    <?php if(isLogged()): ?>
        ...
        <div class="uk-cover uk-margin-top">
                ...
                <?php if(isListen() && !isEpisode()): ?>

                        <?php if(empty($songDetails['iframe'])): ?>

                            <audio id="player" controls>
                                <source src="<?php echo echoOutput($songDetails['link']); ?>" />
                            </audio>

                        <?php endif; ?>

                        <?php if(!empty($songDetails['iframe'])): ?>
                            <?php echo $songDetails['iframe']; ?>
                        <?php endif; ?>

                        <?php if($songDetails['downloadable'] == 1): ?>
                            <a class="uk-button uk-button-primary uk-button-active uk-button-large uk-width-1-1 uk-margin-top" href="<?php echo echoOutput($songDetails['link']); ?>" download><i class="uk-icon-download uk-margin-small-right"></i> <?php echo _DOWNLOAD; ?></a>
                        <?php endif; ?>
