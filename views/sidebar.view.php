    <div class="sidebar">
       ...
       <div class="uk-panel uk-margin-large-bottom">
            ...
            <p class="title-divider"><?php echo _TOPSONGS; ?></p>
            <?php foreach($topSongs as $key => $item): ?>
                <div class="uk-grid uk-flex-middle uk-margin-top uk-margin-bottom topitems" data-uk-grid-margin="">
                    <div class="uk-width-medium-1-3 uk-row-first uk-cover-background">
                        <a href="<?php echo $urlPath->song($item['song_id'], $item['song_slug']); ?>">
                            <img src="<?php echo $urlPath->image($item['song_image']); ?>">
                        </a>
                    </div>
                    <div class="uk-width-medium-2-3 uk-flex-middle padding-left">
                        <p class="number uk-text-contrast"><?php echo showOrder($key); ?></p>
                        <a class="uk-link-muted title" href="<?php echo $urlPath->song($item['song_id'], $item['song_slug']); ?>">
                            <?php echo echoOutput($item['song_title']); ?>
                        </a>
                        <p class="rating"><i class="ion-ios-star"></i> <?php echo showRating($item['song_rate']); ?></p>
        ...
        <ul class="uk-nav uk-nav-side uk-nav-parent-icon" data-uk-nav="">
                <li class="uk-parent">
                    <a href="#"><?php echo _SONGSPAGETITLE; ?></a>
                    <ul class="uk-nav-sub">
                        <?php foreach($songsGenres as $item): ?>
                        <li><a href="<?php echo $urlPath->genre('songs', $item['genre_id']); ?>"><?php echo echoOutput($item['genre_title']); ?></a></li>
                        <?php endforeach; ?>
                    </ul>
                </li>
            </ul>
